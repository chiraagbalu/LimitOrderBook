...
while True:
    ...
    for market in markets:
        ...
        print(
            f'{market} | delta: {np.round(100*delta, 3)} | delta_n: {np.round(delta_notional, 3)} | init: {np.round(init_inventory[market[:-4]], 4)} | fade: {np.round(fade, 4)}')
        print(f'{market} | mid: {np.round(mid, 4)} | bidsigma: {np.round(bidsigma, 3)} | asksigma: {np.round(asksigma, 3)}')
